...
| 木槌计数口径（D-01） | 计数在额外触发派发时 +1；木槌读取本次触发开始前的快照（不计入自身） | 结算算法中 `countBefore` 快照位置固定；影响 `木槌→镜子→镜子` 结果（54） | Agent 默认决策（待确认 Q-01） |
| 持久化口径（D-04） | 运行时 localStorage 单键 JSON；SQLite DDL 为规范模型 + 可选适配器 | 适配器层抽象；两实现共享 `saveSchemaVersion` 与迁移链 | Agent 默认决策（待确认 Q-02） |
...
| 存档写入 | 单键整体覆盖（原子替换）+ 写入前 `op_seq` 乐观锁 + 写入后回读校验 | 不采用多键分片写（非原子、易半写入）；不采用 IndexedDB 事务（数据量小、复杂度收益比低） | 写失败回滚；损坏原文转存 `mnh.save.corrupted.*`；损坏不白屏 |
...
| 桌面壳 | Tauri v2 加载本地静态产物；SQLite 作为可选适配器 | 不采用 Electron（体积与安全配置成本更高）；不采用远程加载（违反离线要求） | 导航拦截 + CSP；构建失败不影响浏览器版交付 |
...
│    ├─ LocalStorageAdapter（默认 · 原子单键 + 回读校验 + op_seq 乐观锁）    │
│    └─ SqliteAdapter（可选 · 桌面壳 · 执行 data-model.md 第四章 DDL）       │
└────────────────────────────────────────────────────────────────────────┘
```
...
**为什么单独设计**：存档位于 localStorage / 本地文件，用户可编辑、可截断、可被其他窗口并发写；同时它又是"可复现性"的唯一载体（种子 + seq + 三条流状态）。校验缺失会直接导致白屏或非法状态。
...
| `PersistenceAdapter` | `load/commit` 抽象（含 `op_seq` 乐观锁与回读校验） | `src/platform/` | 表现层、AI 接口 | 禁止静默吞掉写入异常 |

**扩展方式**：新增字段 = 更新快照契约 + 校验清单 + `data-model.md` 字段映射；`saveSchemaVersion` 递增并注册迁移函数与单测样本。

**失败与兜底**：
- 解析/校验/迁移失败 → 转存损坏原文（`mnh.save.corrupted.<时间戳>`，最多 3 份）→ 进入 `save_corrupted` → 首页提示 + 允许新局，不白屏。
- 写入失败 → 回滚本次操作（内存与存档一致）+ 提示 + 重试入口。
...
**关联方式**：全部表间关联使用业务编码（`save_id`、`card_id`、`template_id`），**禁止物理外键**；运行时以 localStorage 单键 JSON 快照承载同一结构（字段映射见 `data-model.md` §6）。
...
> 运行时默认载体为 localStorage 单键 JSON（无 SQL 查询）；SQLite 仅在桌面壳可选启用（D-04）。因此索引仅按"若启用 SQLite 适配器"的实际访问场景评估，遵循最小索引原则。
...
| # | 风险 | 概率 | 影响 | 应对 |
| --- | --- | --- | --- | --- |
| R1 | D-01 木槌计数口径与策划预期不一致（54 vs 90） | 中 | 中 | 判定集中在 `countBefore` 单点，改判 1 行；TC-006 锁定当前口径；编码前确认 Q-01 |
| R2 | 桌面壳持久化口径未定（localStorage vs SQLite 唯一） | 低 | 中 | D-04 统一口径 + `PersistenceAdapter` 抽象；切换仅影响适配器层 |
...
| R6 | localStorage 被禁用/配额不足 | 低 | 中 | 回滚 + 明确提示 + 重试入口；不静默失败 |
...
| R11 | 桌面壳工具链（Rust）在交付环境不可用 | 中 | 低 | M6 收尾且可后置；浏览器版与内核交付不受影响（PRE-04） |
...
| R13 | 浏览器与桌面壳存档不互通 | 同一玩家在两个环境进度独立 | 两者 localStorage 相互独立（不同 origin），PRD 未要求互通（NG-010） |
...
> 基于：《午夜落槌 · 藏品连锁》从0到1产品需求文档（2026-09-20）；`../midnight-hammer-spec.md`；`../data-model.md`；`../task-split/task.md`；`../task-split/decision-log.md`；用户 Human In Loop 决策 U-01–U-05。
