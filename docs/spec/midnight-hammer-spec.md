...
| 用户 Human In Loop 回答（2026-09-20，共 2 轮） | 第 1 轮 5 项：交付范围、技术栈、持久化、数据模型产物形态、桌面壳方案；第 2 轮 3 项：进入编码授权、木槌计数口径、桌面壳持久化载体、规则弹窗内容 | 高 | 8 项决策已全部确认，见 §19.1、§19.4 |
...
| NG-008 | 服务端数据库与后端服务 | 用户已确认（U-03/U-07）：无服务端、无数据库服务；持久化按载体分工——浏览器版为 localStorage 单键 JSON，桌面壳为本地 SQLite 文件（唯一载体），见 D-04、§12 |
| NG-009 | 正式插画与音频资源 | PRD §14 明确"不强制"；本 Spec 只要求功能状态具备清晰视觉反馈 |
| NG-010 | 存档在浏览器与桌面壳之间的互通 | 两者载体相互独立（浏览器 localStorage 键 vs 桌面壳本地 SQLite 库文件），PRD 未要求互通 |
...
| 存档并发 | 浏览器版：存储中的 `seq` 必须等于内存 `seq`；桌面壳：`mh_save_slot.op_seq` 必须等于内存 `opSeq`（事务内比对） | `SAVE_CONFLICT` |
| 存档写入 | 浏览器版 `setItem` 成功且回读校验通过；桌面壳 SQLite 事务 `COMMIT` 成功且提交后回读一致（§12.3.1） | `SAVE_WRITE_FAILED` |
...
└──────────────────────────────────────────────────────────────┘
┌──────────────────────────────────────────────────────────────┐
│  持久化适配  src/platform/（按载体二选一，同一 saveSchemaVersion）│
│  LocalStorageAdapter（浏览器版唯一载体 · 单键原子替换 + 回读校验）│
│  SqliteAdapter（桌面壳唯一载体 · 单事务写 data-model.md 同一 DDL）│
└──────────────────────────────────────────────────────────────┘
```
...
| 持久化适配 | `src/platform/` | 存档读写、并发检测、写入回读校验、迁移调度；**按载体二选一**：浏览器版 `LocalStorageAdapter`、桌面壳 `SqliteAdapter`（U-07） | `src/kernel/` 的序列化契约 | 禁止实现业务规则；禁止静默吞掉写入错误；**禁止同一载体同时启用两个适配器**（桌面壳不得回退到 localStorage） |
| 桌面壳 | `src-tauri/`（Tauri v2 配置与最小 Rust 壳） | 加载本地构建产物、以 SQLite 为唯一持久化载体（U-07）、禁止外部导航与新窗口、单窗口运行 | 无 | 禁止开启远程加载；禁止暴露额外命令面；**禁止在桌面壳内启用 localStorage 存档路径** |
...
| 编号 | 判定点 | 结论 | 依据 |
|------|--------|------|------|
| D-01 | 额外触发计数与木槌读取时机 | 计数在额外触发事件**派发时**立即 +1；能力读取**本次触发事件开始前**的快照，因此木槌被额外触发时**不**计入自身这一次（`木槌4→镜子2→镜子2` = 18 点 × 3 热度 = **54**） | **已由用户确认（U-06，2026-09-20）**。依据 PRD §9.2「木槌读取自己本次触发前的额外触发次数」为针对木槌的具体规则（具体优先于一般）；PRD §8.3「立即加1」描述计数本身的推进时机。该口径可逐位复现 PRD §8.4 三个示例；若改判为"计入自身"则为 `18点×5热度=90` |
...
| 并发写来源 | 浏览器版：同一页面内的串行命令 + 多标签页共享同一 localStorage 键；桌面壳：单窗口内的串行命令（SQLite 文件由单一进程独占，仍以 `op_seq` 乐观校验兜底） | 无跨进程事务需求 |
| 状态真相源 | 内存中的不可变 `RunState`；存档是它的持久化镜像 | 任何界面展示都从 `RunState` 派生 |
| 事务边界 | **一次持久化命令 = 一次快照提交**（校验 → 计算 → 落盘 → 回读校验 → 提交内存）；落盘原子性按载体实现：浏览器版为单键整体替换，桌面壳为**单个 SQLite 事务**（U-07）；排列编辑（ACTION-003）属持久化命令，撤换选择（ACTION-004）属 UI 瞬时态 | 见 §12.3、§12.3.1 |
| 一致性等级 | 强一致（内存与存档在同一提交内一致，排列编辑同样落盘，因此**内存与存档在任意已提交时刻恒等**）；浏览器版跨标签页为乐观并发控制，桌面壳由单进程 + 单事务保证 | 见 §12.4 |
...
handleCommand(command):
  ...
  4) 落盘：snapshot = serialize(nextState)
          ...
          载体分支：浏览器版 = localStorage 单键整体替换；
                    桌面壳 = 单个 SQLite 事务内整表替换（见 §12.3.1）
  5) 提交：内存 state = nextState；返回视图
  任一环节失败 → 保持原 state 不变，返回对应错误码
```

**§12.3.1 桌面壳 SQLite 落盘契约（U-07，强制）**

| 项 | 约定 |
|----|------|
| 载体 | 桌面壳**强制**以 SQLite 为唯一持久化载体；执行 `data-model.md` 第四章 DDL；**不启用 localStorage 存档路径** |
| 库文件位置 | 应用数据目录下的单一库文件（如 `midnight-hammer.db`），不使用用户可任意指定的外部路径（SEC-08） |
| 写入原子性 | 整次提交在**一个事务**内完成：`BEGIN IMMEDIATE` → 更新 `mh_save_slot` 行 → 整表替换子表（`mh_save_line_slot`、`mh_save_reward_candidate`、`mh_save_settlement_event` 按 `save_id` 删除后重插）→ 追加 `mh_run_history`（仅结束时）→ `COMMIT`；任一步失败即 `ROLLBACK`，内存状态不变 |
| 幂等与并发 | 事务内先 `SELECT op_seq FROM mh_save_slot WHERE save_id = ?` 与内存 `opSeq` 比对，不一致则 `ROLLBACK` 并返回 `SAVE_CONFLICT` |
| 回读校验 | `COMMIT` 后按 `save_id` 重新查询并重建快照，与 `nextState` 逐字段比对；不一致按 `SAVE_WRITE_FAILED` 处理 |
| 迁移 | 与浏览器版**共享同一 `saveSchemaVersion` 与迁移链**；迁移在同一事务内完成，失败即回滚并走损坏流程 |
| 损坏处理 | 库文件不可打开 / 结构校验失败 → 转存原库文件为 `save.corrupted.<ISO 时间戳>.db`（最多 3 份），进入 `save_corrupted` |
| 字段映射 | 与快照 JSON 1:1 映射（`data-model.md` §6.2/§6.3）；语义枚举 ↔ 数值枚举转换在 `src/platform/` 完成 |

> 说明：由于桌面壳为单进程单窗口，SQLite 事务本身已保证写入原子性，`op_seq` 校验用于兜底"同机二次启动/多实例"等异常场景；浏览器版与桌面壳的**规则语义、快照结构与迁移链完全一致**，差异仅限 `src/platform/` 适配器实现（D-04 修订后口径）。
...
| 多标签页并发写（浏览器版） | 写入前比对存储中快照的 `seq` 与内存 `seq`；不一致即拒绝 | 返回 `SAVE_CONFLICT`，提示"检测到其他窗口的进度变更，请重新载入"；**不静默覆盖**（D-10） | §12.5 |
| 桌面壳多实例/重入（SQLite） | 事务内比对 `mh_save_slot.op_seq` 与内存 `opSeq`；不一致即 `ROLLBACK` | 返回 `SAVE_CONFLICT`，提示重新载入；桌面壳默认单窗口，异常场景由该校验兜底 | §12.3.1、§12.5 |
...
> **适用范围**：本节仅适用于**浏览器版**（localStorage）。桌面壳强制 SQLite（U-07）且默认单窗口，冲突由 §12.3.1 的事务内 `op_seq` 校验兜底，不适用本节的键值比对方案。
...
| SEC-08 | 桌面壳 SQLite 文件边界 | 库文件固定置于应用数据目录，不接受用户指定任意路径；不开放 SQL 直传接口，SQL 语句全部为代码内固定文本（参数化绑定），禁止字符串拼接 | REQ-020.5、U-07 |
...
| 存档快照 | 浏览器版：localStorage 单键；桌面壳：SQLite 库文件（`mh_save_*` 表） | 完整快照 | 每次提交覆盖（浏览器单键替换 / 桌面壳单事务整表替换） | 唯一持久化真相 |
...
| 存档写入 | 同步（浏览器：`setItem` + 回读校验；桌面壳：SQLite 单事务 + 提交后回读） | 每次命令提交 | 失败即回滚操作（§12.3、§12.3.1）；不做异步队列，避免顺序错乱 |
...
| FR-05 | 存档写入失败：浏览器版（配额/隐私模式/被禁用）；桌面壳（磁盘满/库文件被占用或只读/事务失败） | 写入抛错、事务失败或回读不一致 | **回滚本次操作**（内存与存档均不变） | 提示"无法保存进度，操作已撤销" + 载体对应原因（"请检查浏览器存储权限" / "请检查磁盘空间与文件权限"），提供重试 | REQ-014.6、NFR-010 |
...
1. 读取失败 → 转存损坏原文（最多保留 3 份，超出删除最旧）：浏览器版转存为独立键 `mnh.save.corrupted.<ISO 时间戳>`；桌面壳转存为库文件副本 `save.corrupted.<ISO 时间戳>.db`。
...
| 桌面壳 | Tauri v2，加载本地静态构建产物，强制 SQLite 持久化（U-07）；不依赖系统 WebView 与 SQLite 之外的运行时 |
| 分辨率 | 窗口 ≥ 1000×700 无横向滚动；更小窗口允许纵向滚动但不得遮挡核心操作（D-12） |
| 存档互通 | 浏览器（localStorage）与桌面壳（本地 SQLite 文件）载体相互独立，不做互通（NG-010） |
| 数据格式 | 浏览器版存档为 JSON 文本；桌面壳存档为 SQLite 库文件，执行 `data-model.md` 中同一份 DDL（U-07、D-04） |
...
| TC-051 | 桌面壳 SQLite 建表 | 按 `data-model.md` 第四章 DDL 建表成功，12 条模板种子数据入库 | 桌面壳冒烟测试（断言 7 表 10 索引 + 模板行数与 REQ-006 一致） |
| TC-052 | 桌面壳持久化载体唯一性 | 桌面壳运行期间不写入 localStorage 存档键；重启后从 SQLite 恢复同一快照 | 集成测试（断言 `mnh.save.auto` 未被写入 + 重启恢复） |
| TC-053 | 桌面壳事务原子性 | 提交过程中注入失败（模拟磁盘满/占用），事务回滚且内存状态不变 | 集成测试（mock 事务失败，断言 `mh_save_slot` 与内存均未变更） |
| TC-054 | 双载体快照一致性 | 同一快照经 localStorage 与 SQLite 两载体往返后逐字段等价（含 `op_seq`/随机流/流水） | 适配器对照测试（同一 `nextState` 双写双读比对） |
...
| U-04 | 数据模型产物形态 | **仍产出 SQLite 建表 DDL（CREATE TABLE）** | 产出 `data-model.md`：SQLite DDL + JSON 字段映射 + 迁移规则；**该 DDL 即桌面壳的生产建表脚本**（U-07 后不再是"可选适配器参考"） |
| U-05 | PC 桌面离线壳 | **Tauri v2**（Rust 壳，体积小，可禁外部导航） | 桌面壳直接加载 Vite 静态产物；安全策略见 SEC-03、SEC-08 |
| U-06 | 木槌额外触发计数口径（Q-01） | **采纳默认**：木槌被额外触发时**不计入自身**这一次（`木槌→镜子→镜子` = 18 点 × 3 热度 = **54**） | 结算算法 `countBefore` 快照位置锁定；TC-006 期望值 54 生效；规则弹窗示例沿用 PRD §13.1 的 36 分示例 |
| U-07 | 桌面壳持久化载体（Q-02） | **桌面壳强制以 SQLite 为唯一持久化载体**（不使用 localStorage）；执行 `data-model.md` 第四章 DDL | 新增 §12.3.1 落盘契约与 SEC-08；`SqliteAdapter` 由"可选"升级为**必需**（T-02-06 必交付，TC-051–TC-054 覆盖）；浏览器版仍为 localStorage；两载体共享同一 `saveSchemaVersion` 与迁移链（详见 §19.4 Q-02 落地位置） |
| U-08 | 规则弹窗内容范围（Q-03） | **采纳默认**：仅收录 PRD §13.1 要求的固定内容，不额外收录数值曲线说明 | 规则弹窗文案范围锁定，无实现影响 |
| U-09 | 阶段流转授权 | **授权进入「AI-编码」阶段**（2026-09-20） | Spec 冻结；本节点交付后由编码节点按本 Spec 实施 |

> **U-03 / U-04 / U-07 的口径统一（D-04，已按第 2 轮确认修订）**：三者不冲突——**浏览器版**持久化采用 localStorage 单键 JSON（U-03）；**桌面壳**强制以 SQLite 库文件为唯一持久化载体（U-07），执行 `data-model.md` 第四章 DDL（U-04）；两载体字段 1:1 映射，共享同一 `saveSchemaVersion`、同一迁移链与同一快照语义。载体差异被限制在 `src/platform/` 适配器实现，内核、AI 接口与界面完全共用；桌面壳内**不得**启用 localStorage 存档路径（§12.3.1、SEC-08）。

## 19.2 默认决策（Agent 依据 PRD 语义与工程惯例推断）

| 编号 | 决策点 | 结论 | 依据 | 影响 | 验证方式 |
|------|--------|------|------|------|----------|
| D-01 | 额外触发计数与木槌读取时机 | 计数在额外触发**派发时**立即 +1；能力读取**本次触发开始前**的快照 → 木槌被额外触发时不计入自身 | PRD §9.2 为具体规则；可逐位复现 PRD §8.4 与 §19.1 三个示例。**已由用户确认（U-06）**，不再是待确认默认值 | 决定 `木槌→镜子→镜子` 的结果（54 而非 90） | TC-001/002/003/006 |
...
| D-04 | 持久化双载体口径（**已按 U-07 修订**） | **浏览器版 = localStorage 单键 JSON（唯一载体）；桌面壳 = SQLite 库文件（唯一载体，强制）**；两载体执行同一份规范模型与同一 `saveSchemaVersion`/迁移链，字段 1:1 映射 | U-03（localStorage）+ U-04（SQLite DDL）+ **U-07（桌面壳强制 SQLite）**；PRD §18 要求浏览器与桌面均可离线运行 | 载体差异被限制在 `src/platform/` 适配器层；内核、AI 接口、界面完全共用；桌面壳不再有 localStorage 路径 | 迁移单测、TC-051–TC-054、桌面壳冒烟 |
...
|------|------|------|------|--------------|------|
| R-01 | D-01 口径与策划预期不一致（木槌被额外触发时是否计入自身） | 低 | 中：个别组合成交价不同（54 vs 90） | **已由用户确认（U-06）：不计入自身 → 54**，风险关闭；TC-006 已锁定该口径 | **已关闭**（U-06） |
| R-02 | 双载体实现分叉（浏览器 localStorage 与桌面壳 SQLite 行为不一致） | 中 | 中：同一操作在两载体下恢复结果不同 | **已由 U-07 定调**：两载体共享同一 `saveSchemaVersion`、同一迁移链与同一字段映射；以 TC-054（双载体快照往返等价）作为防分叉门禁；差异仅允许存在于 `src/platform/` | 已缓解（U-07、TC-054） |
...
| R-06 | 存档写入失败：浏览器版 localStorage 被禁用/配额不足；桌面壳 SQLite 磁盘满/库文件被占用或只读 | 低 | 中：无法保存 | FR-05 回滚 + 载体对应提示；不静默失败；桌面壳另由单事务回滚保证不半写（§12.3.1） | 已缓解 |
| R-07 | 存档被外部篡改导致崩溃 | 低 | 中：白屏 | SEC-04 全量校验 + FR-02 损坏流程 | 已缓解 |
| R-08 | 浏览器兼容差异导致动画异常 | 低 | 低：视觉降级 | FR-13 动画层 try/catch 降级为直接展示 | 已缓解 |
| R-11 | 桌面壳 SQLite 适配器实现复杂度上升（事务、整表替换、枚举转换、迁移） | 中 | 中：M2/M6 工作量增加 | 契约已固化在 §12.3.1 与 `data-model.md` §6.6；DDL 已实测可执行（附录 A）；以 TC-051–TC-054 覆盖；适配器仅依赖内核序列化契约，不侵入内核 | 已缓解（待实现） |
...
| 编号 | 原待确认内容 | 用户最终决策（2026-09-20） | 落地位置 |
|------|--------------|---------------------------|----------|
| Q-01 | D-01 木槌读取额外触发计数的口径 | **采纳默认：不计入自身**（`木槌→镜子→镜子` = 18 点 × 3 热度 = 54） | §11.6 D-01、§19.1 U-06、TC-006 |
| Q-02 | 桌面壳是否强制以 SQLite 为唯一持久化载体 | **强制 SQLite**（桌面壳不使用 localStorage） | §12.3.1、§13.2 SEC-08、§19.1 U-07、D-04、`data-model.md` §6.6、T-02-06 |
| Q-03 | 规则弹窗是否额外收录数值曲线说明 | **采纳默认：不收录**，仅收录 PRD §13.1 要求内容 | §19.1 U-08、REQ-017、TC-065 |

> 截至 2026-09-20，Spec 无未决项。所有默认决策（D-01–D-15）已冻结；其中 D-01 经用户确认、D-04 经用户修订（U-06/U-07）。
...
| DEL-08 | PC 桌面壳离线版本 | Tauri v2 壳 + 安全策略 + **强制 SQLite 持久化载体**（U-07） | REQ-020.4、SEC-03、SEC-08、§12.3.1 |
...
| PRE-02 | ~~Q-01 口径确认~~ **已完成（U-06）**：不计入自身 → 54 | TC-006 期望值锁定为 54 |
| PRE-03 | 技术栈落地确认 | TypeScript + Vite + React 18 + Vitest；内核测试运行在 node 环境 |
| PRE-04 | 桌面壳工具链可用 | Tauri v2 需要 Rust 工具链，且桌面壳强制 SQLite（U-07）需确认 Tauri SQL 插件可用（或用最小 Rust 侧 `rusqlite` 命令面）；若环境缺失，浏览器版与内核不受影响，桌面壳作为最后一步 |
...
| PRE-07 | 阶段流转授权 | **已获授权进入「AI-编码」阶段（U-09）**；编码节点按本 Spec 实施，不得偏离 U-06/U-07/U-08 结论 |
...
| TC-006 | 封印木槌 → 裂纹镜 → 裂纹镜 | 18 点 / 3 热度 / 54 成交（触发事件 6） | 由 D-01 口径推导 | ✅ 与 D-01 口径一致；**该口径已由用户确认（U-06）** |
...
| REQ-014 | §12.3.1、§15、§17.2 | T-02-01–T-02-04、T-02-06 | TC-040–TC-043、TC-049、TC-051–TC-054 | `mh_save_*` 表 / 单键 JSON / SQLite 库文件 | PRD §15、U-07 |
...
| REQ-020 | §10.1、§12.3.1、§13.2、§14 | T-06-01、T-06-02 | TC-050–TC-052、TC-070–TC-074 | Tauri 配置、CSP、SQLite 库文件位置 | PRD §18、U-07 |
...
| NFR-008 | §13.2 SEC-03、SEC-08 | T-06-02 | TC-052、TC-074 | Tauri 安全配置、SQLite 文件边界 | PRD §18、U-07 |
| NFR-009 | §13.2 SEC-04 | T-02-02 | TC-041 | 校验器 | PRD §15.3 |
| NFR-010 | §12.3、§12.3.1 | T-02-04、T-02-06 | TC-042、TC-053 | 回读校验 / 事务回滚 | PRD §15.1、U-07 |
